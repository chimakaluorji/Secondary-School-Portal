...
if ($retval != 0) {
    ...
    if ($return_position == 0) {
        ...
        $stmt = $pdo->prepare("UPDATE resultsubject SET total = ?,position = ?,grade = ?,remark = ? WHERE regnumber = ? and subjectid = ? and sessionid = ? and termid = ? and classid = ? and classdemarcationid = ?");

        $stmt->execute([$total, $position, $grade, $remark, $regnumber, $subjectid, $sessionid, $termid, $classid, $classdemarcationid]);
        ...
        $stmt->execute([$total, $return_position, $grade, $remark, $regnumber, $subjectid, $sessionid, $termid, $classid, $classdemarcationid]);
