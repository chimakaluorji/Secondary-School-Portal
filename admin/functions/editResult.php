...
if ($retval != 0) {
    ...
    $stmt = $pdo->prepare("UPDATE result SET caone = ?,catwo = ?,cathree = ?,exam = ?,total = ? where regnumber = ? and subjectid = ? and sessionid = ? and termid = ? and classid = ? and classdemarcationid = ?");

    $stmt->execute([$caone, $catwo, $cathree, $exam, $total, $regnumber, $subjectid, $sessionid, $termid, $classid, $classdemarcationid]);
